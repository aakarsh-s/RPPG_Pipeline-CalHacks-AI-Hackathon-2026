...
Keep this terminal running while the browser UI is open. A terminal can only run one foreground program at a time. If you previously ran the CLI webcam command:
...
If you are using VS Code, run this from the integrated terminal or create a launch configuration that uses `"module": "rppg.web"`. Do **not** run `rppg/web.py` or `rppg/pipeline.py` as standalone files, because these files are part of the `rppg` Python package and use relative imports.

### Why `python -m` matters

This project is structured as a package:
...
That leading dot means “import from the current package.” Python only knows the current package when you run from the project root with module syntax:
...
If VS Code runs the file directly like this:

```bash
python rppg/pipeline.py
```

Python treats `pipeline.py` as an isolated script, not as part of the `rppg` package. Then relative imports fail with:

```text
ImportError: attempted relative import with no known parent package
```

The fix is not to change the imports; the fix is to run the module in package context.
...
```json
{
  ...
  "configurations": [
    ...
    {
      "name": "Run rPPG CLI Webcam",
      "type": "python",
      "request": "launch",
      "module": "rppg",
      "cwd": "${workspaceFolder}",
      "console": "integratedTerminal",
      "args": ["--source", "0", "--method", "pos", "--show"]
    }
  ]
}
```

Click **start session**. Your browser will ask for camera permission. Once accepted, the browser captures webcam frames and sends them to the local Python backend. The backend runs the rPPG pipeline and returns the live annotated camera frame plus heart-rate status.
...
- Do not use `https://localhost`; use plain `http://127.0.0.1:8000`.
- Make sure the terminal says something like `Running on http://127.0.0.1:8000`.
...
- Stop any old `python -m rppg` or `python -m rppg.web` process with `Ctrl+C`.
- Refresh the page and click **start session** again.
...
If VS Code runs `rppg/pipeline.py` directly, it bypasses Python’s package context and can trigger a relative-import error. Use `python -m rppg` from the project root instead.
